...
export default function AdminPage({ label, role, deviceId, onLogout, onBack }: AdminPageProps) {
  ...
  async function handleShiftSubmit(e: React.FormEvent) {
    e.preventDefault();
    if (startDate && endDate && new Date(startDate) > new Date(endDate)) { setShiftMsg({ type: 'err', text: `'To' date cannot be before 'From' date` }); return; }
    setShiftMsg(null); setShiftBusy(true);
    try {
      ...
      <main className="flex-1 overflow-hidden flex max-w-7xl mx-auto w-full">
        ...
        <div className="flex-1 overflow-y-auto bg-white p-4">
              ...
              <div className="bg-white rounded-xl border border-indigo-100 overflow-hidden shadow-sm">
                ...
                {loading ? <div className="text-center py-6 text-xs text-indigo-400">Loading...</div> : visibleUsers.length===0 ? <div className="text-center py-6 text-xs text-indigo-400">No users yet</div> : (
                  <div className="overflow-x-auto"><table className="w-full"><thead><tr className="bg-indigo-50/80"><th className="px-4 py-2 text-left text-[10px] font-semibold text-indigo-600 uppercase">Username / Recovery Email</th><th className="px-4 py-2 text-left text-[10px] font-semibold text-indigo-600 uppercase">Label</th><th className="px-4 py-2 text-left text-[10px] font-semibold text-indigo-600 uppercase">Role</th><th className="px-4 py-2 text-left text-[10px] font-semibold text-indigo-600 uppercase">Unit</th><th className="px-4 py-2 text-right text-[10px] font-semibold text-indigo-600 uppercase">Action</th></tr></thead><tbody className="divide-y divide-indigo-100">{visibleUsers.map(u=> editingUser===u.username ? (
                    <tr key={u.username} className="bg-indigo-50/80"><td className="px-4 py-2 text-xs"><div>{u.username}</div><input type="email" value={editForm.email} onChange={e=>setEditForm({...editForm,email:e.target.value})} placeholder="Recovery email" className="mt-1 w-full px-2 py-1 border border-indigo-200 rounded text-xs bg-white"/></td><td className="px-4 py-2"><input value={editForm.label} onChange={e=>setEditForm({...editForm,label:e.target.value})} className="w-full px-2 py-1 border border-indigo-200 rounded text-xs bg-white"/><input type="password" value={editForm.password} onChange={e=>setEditForm({...editForm,password:e.target.value})} placeholder="New password (optional)" autoComplete="new-password" className="mt-1 w-full px-2 py-1 border border-indigo-200 rounded text-xs bg-white"/></td><td className="px-4 py-2"><select value={editForm.role} onChange={e=>setEditForm({...editForm,role:e.target.value})} className="w-full px-2 py-1 border border-indigo-200 rounded text-xs bg-white"><option value="manager">Manager</option>{isSuper && <option value="admin">Admin</option>}</select></td><td className="px-4 py-2"><select value={editForm.device_id} onChange={e=>setEditForm({...editForm,device_id:parseInt(e.target.value)})} className="w-full px-2 py-1 border border-indigo-200 rounded text-xs bg-white">{locations.map(loc=> <option key={loc.device_id} value={loc.device_id}>{loc.name}</option>)}</select></td><td className="px-4 py-2"><div className="flex items-center gap-1 justify-end"><button onClick={()=>setEditingUser(null)} className="p-1 rounded hover:bg-gray-100"><X size={12}/></button><button onClick={handleEdit} className="text-[10px] bg-indigo-500 text-white px-2 py-0.5 rounded">Save</button></div></td></tr>
                  ) : (
          ...
          {adminTab === 'shifts' && (
            <div className="max-w-3xl mx-auto space-y-3">
              <div className="bg-white rounded-xl border border-indigo-100 p-4 shadow-sm">
                <h2 className="text-xs font-bold text-indigo-800 mb-1 flex items-center gap-2"><Clock size={14} className="text-indigo-600"/> Shift Management</h2>
                <p className="text-[10px] text-indigo-500 mb-1">No coding needed. Assign any shift to any employee for any dates. It appears <span className="font-bold text-orange-600">instantly</span> in that employee&apos;s Monthly data (orange <span className="inline-flex items-center px-1 py-0 rounded bg-orange-100 border border-orange-200 text-[8px]">Roster</span> badge on Shift column).</p>
                <p className="text-[9px] text-gray-500 mb-3">Example: Today 25 → assign <span className="font-semibold">Night 21:00-06:00</span> to 2 people for <span className="font-semibold">27-28 Sep</span> → open Monthly view for those employees → 27 & 28 show <span className="font-bold text-orange-700">Night Roster</span> even before they punch.</p>
                ...
                {shiftMsg?.type==='ok' && (<div className="text-[10px] px-2.5 py-1.5 rounded-lg mb-2 bg-blue-50 text-blue-700 border border-blue-200">✅ Roster saved! Go to <span className="font-bold">Dashboard → Monthly Summary → click employee name</span> → check Shift column for orange Roster badge on {startDate || 'selected'} → {endDate || 'ongoing'}. Future dates show instantly, past dates affect Late/Present counts.</div>)}
                <form onSubmit={handleShiftSubmit} className="space-y-2.5">
                  ...
                  <div><label className="text-[9px] font-bold text-indigo-600 uppercase">3. Assign Employees {selectedEmps.size===0 ? <span className="font-normal normal-case text-gray-500">(leave empty = whole unit)</span> : null}</label><div className="border border-indigo-200 rounded-lg overflow-hidden"><div className="flex items-center gap-1.5 px-2.5 py-1.5 bg-indigo-50 border-b border-indigo-100"><Search size={10} className="text-indigo-400"/><input value={empSearch} onChange={e=>setEmpSearch(e.target.value)} placeholder="Search name, badge, or ID..." className="flex-1 text-[10px] bg-transparent outline-none"/><button type="button" onClick={toggleAllShiftEmps} className={`text-[9px] font-bold px-1.5 py-0.5 rounded ${allShiftEmpsSelected ? 'bg-indigo-200 text-indigo-800' : 'bg-indigo-100 text-indigo-600'}`}>{allShiftEmpsSelected ? 'Clear':'All'}</button></div><div className="max-h-36 overflow-y-auto">{filteredShiftEmps.length===0 ? <p className="text-[10px] text-indigo-400 py-2 text-center">No employees on this unit</p> : filteredShiftEmps.map(e=> (<label key={e.empid} className={`flex items-center gap-1.5 px-2.5 py-1 border-b border-indigo-50 last:border-0 ${selectedEmps.has(e.empid) ? 'bg-indigo-50' : 'hover:bg-indigo-50/50'}`}><input type="checkbox" checked={selectedEmps.has(e.empid)} onChange={()=>toggleShiftEmp(e.empid)} className="w-3 h-3"/><span className="text-[10px] flex-1 truncate">{e.name}</span><span className="text-[8px] font-mono">ID {e.codeInDevice}</span></label>))}</div><div className="px-2.5 py-1 bg-indigo-50 border-t border-indigo-100 flex justify-between text-[8px]"><span className="text-indigo-400">{empList.length} employees in this unit</span>{selectedEmps.size>0 ? <span className="font-bold text-indigo-600">{selectedEmps.size} selected → roster shows only for them</span> : <span className="italic text-indigo-500">No one selected = shift applies to ALL employees of this unit</span>}</div></div></div>
                  {/* Live preview for non-technical clients */}
                  <div className="px-2.5 py-2 rounded-lg bg-gray-50 border border-gray-200 text-[10px] leading-relaxed">
                    <span className="font-bold text-gray-700">Preview:</span> <span className="font-bold text-indigo-700">{activePreset} {startTime}–{endTime}</span> → {selectedEmps.size>0 ? <span className="font-bold text-indigo-700">{selectedEmps.size} employee(s)</span> : <span className="font-bold">all {empList.length} in unit</span>} {startDate ? `from ${startDate}` : ''} {endDate ? ` to ${endDate}` : startDate ? ' onwards' : ' (no dates = forever — shows on every day)'} <span className="text-orange-600 font-semibold">• appears instantly as orange Roster badge in Monthly view</span>
                  </div>
                  {startDate && endDate && new Date(startDate) > new Date(endDate) && (<div className="text-[10px] px-2.5 py-1.5 rounded-lg bg-red-50 text-red-600 border border-red-200 flex items-center gap-1"><AlertCircle size={11}/> &apos;To&apos; date cannot be before &apos;From&apos; date</div>)}
                  ...
                  <button type="submit" disabled={shiftBusy || (!!startDate && !!endDate && new Date(startDate) > new Date(endDate))} className={`w-full py-1.5 rounded-lg text-xs font-bold text-white flex items-center justify-center gap-1.5 disabled:opacity-50 ${editingShiftId!==null ? 'bg-gradient-to-r from-amber-500 to-amber-600' : 'bg-gradient-to-r from-indigo-500 to-indigo-600'}`}>{shiftBusy ? <Loader2 size={12} className="animate-spin"/> : editingShiftId!==null ? <Pencil size={12}/> : <CalendarPlus size={12}/>}{shiftBusy ? (editingShiftId!==null ? 'Updating…' : 'Adding…') : editingShiftId!==null ? 'Update Rule' : selectedEmps.size>0 ? `Add Night/Morning for ${selectedEmps.size} employee(s)` : 'Add for Whole Unit (all employees)'}</button>
                  <p className="text-[9px] text-center text-gray-500">Tip: After saving, open <span className="font-semibold">Dashboard → click employee name → Daily Punch Log</span> — future dates 27-28 will show Shift = <span className="inline-flex items-center px-1 py-0 rounded bg-orange-100 border border-orange-200 text-[9px] font-bold text-orange-800">Night Roster</span></p>
                </form>
